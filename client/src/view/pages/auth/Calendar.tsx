...
function ServerDay(props: PickersDayProps<Dayjs> & { events: Post[] }) {
  ...
  const eventsForDay = useMemo(() => {
    return listEvent.filter((event) => event?.date === props.day.format('DD-MM-YYYY'));
  }, [listEvent, props.day]);
        ...
        isEventDay ? (
          <div>
            {eventsForDay.map((event, index) => (
              <div key={index}>
                <div>{event.title} בשעה: {event.time}</div>
              </div>
            ))}
          </div>
        ) : (
...
export default function Calendar({ events }: { events: Post[] }) {
  ...
  return (
    <Card style={{ textAlign:'center' }}>
                <CardHeader title="לוח האירועים שלי"/>
